...
counts = ary.length / 6
for (let r=0; r<counts; r++) this._colors.push(fullColorHex(r*255/counts,       255,         0));
for (let g=counts; g>0; g--) this._colors.push(fullColorHex(      255, g*255/counts,         0));
for (let b=0; b<counts; b++) this._colors.push(fullColorHex(      255,         0, b*255/counts));
for (let r=counts; r>0; r--) this._colors.push(fullColorHex(r*255/counts,         0,       255));
for (let g=0; g<counts; g++) this._colors.push(fullColorHex(        0, g*255/counts,       255));
for (let b=counts; b>0; b--) this._colors.push(fullColorHex(        0,       255, b*255/counts));
for (let r=0; r<=ary.length-this._colors.length; r++) colors.push(fullColorHex(r*255/counts,       255,         0));
this._colors.push(fullColorHex(        0,       255,         0));
  ...
  AnimatedArray.prototype._step = function() {
    ...
    var currI =  this._colors[i];
    var currJ =  this._colors[j];
    if (action[0] === 'compare') {
